...
Current high-end GPUs for the consumer market tops out at 24GB *VRAM*. This memory is speedier than system memory, and as the LLM processing now happens on the GPU, processing speed is higher (GPUs are better then CPUs at the type of math involved in LLMs) and latency is also lower. This allows for running decent sized ("quality") models at decent speed ("performance"). GPUs with less memory (than the top-end GPUs) mostly also have less memory bandwidth. I.e. they are slower. But still faster than your system memory (DIMMs in your motherboard).
